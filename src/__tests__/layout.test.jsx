...
describe('Sidebar', () => {
  ...
  it('all sidebar link hrefs start with "/"', () => {
    renderSidebar();
    const links = screen.getAllByRole('link');
    links.forEach((link) => {
  ...
  it('sidebar link text matches the href path segment', () => {
    renderSidebar();
    const links = screen.getAllByRole('link');
    links.forEach((link) => {
      const href = link.getAttribute('href'); // e.g. "/dashboard"
      expect(link.textContent).toBe(href.slice(1));   // e.g. "dashboard"
    });
  });
});

// ---------------------------------------------------------------------------
// BottomNav — additional
// ---------------------------------------------------------------------------
describe('BottomNav — additional', () => {
  const renderBottomNav = () =>
    render(
      <MemoryRouter>
        <BottomNav />
  ...
  it('nav has left: 0 inline style', () => {
    renderBottomNav();
    const nav = screen.getByRole('navigation');
    expect(nav).toHaveStyle({ left: '0px' });
  });

  it('nav has right: 0 inline style', () => {
    renderBottomNav();
    const nav = screen.getByRole('navigation');
    expect(nav).toHaveStyle({ right: '0px' });
  });

  it('all BottomNav link hrefs start with "/"', () => {
    renderBottomNav();
