...
describe('VoicePlayer', () => {
  ...
  it('handles an empty string text without crashing', () => {
    render(<VoicePlayer text="" />);
    // text?.slice(0,140) on "" is "" — result is "Voice Script: "
    expect(screen.getByText(/Voice Script:/)).toBeInTheDocument();
  });

  it('never renders more than 140 characters of text', () => {
    const longText = 'Z'.repeat(500);
    render(<VoicePlayer text={longText} />);
    const el = screen.getByText(/Voice Script:/);
    // textContent includes "Voice Script: " prefix (14 chars) + max 140 chars
    expect(el.textContent.length).toBeLessThanOrEqual(14 + 140);
  });
});

// ---------------------------------------------------------------------------
// ImageGenerator — additional edge cases
// ---------------------------------------------------------------------------
describe('ImageGenerator — additional', () => {
  it('renders a new image when imageUrl prop changes', () => {
    const { rerender } = render(<ImageGenerator imageUrl="https://example.com/a.png" />);
    expect(screen.getByRole('img')).toHaveAttribute('src', 'https://example.com/a.png');

    rerender(<ImageGenerator imageUrl="https://example.com/b.png" />);
    expect(screen.getByRole('img')).toHaveAttribute('src', 'https://example.com/b.png');
  });

  it('removes image from DOM when imageUrl becomes falsy after being set', () => {
    const { rerender, container } = render(<ImageGenerator imageUrl="https://example.com/img.png" />);
    expect(screen.getByRole('img')).toBeInTheDocument();

    rerender(<ImageGenerator imageUrl={null} />);
    expect(container.firstChild).toBeNull();
  });
});

// ---------------------------------------------------------------------------
// VideoPreview — additional edge cases
// ---------------------------------------------------------------------------
describe('VideoPreview — additional', () => {
  it('renders a new video when src prop changes', () => {
    const { rerender } = render(<VideoPreview src="https://example.com/a.mp4" />);
    expect(document.querySelector('video')).toHaveAttribute('src', 'https://example.com/a.mp4');

    rerender(<VideoPreview src="https://example.com/b.mp4" />);
    expect(document.querySelector('video')).toHaveAttribute('src', 'https://example.com/b.mp4');
  });

  it('removes video from DOM when src becomes falsy after being set', () => {
    const { rerender, container } = render(<VideoPreview src="https://example.com/video.mp4" />);
    expect(document.querySelector('video')).toBeInTheDocument();

    rerender(<VideoPreview src={null} />);
    expect(container.firstChild).toBeNull();
  });
});